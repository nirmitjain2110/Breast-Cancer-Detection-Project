...
pred_openai_nano_zs_path = "features_openai_4.1nano_zs.xlsx"

df_gt = pd.read_excel(gt_path)
df_pred_openai_nano_zs = pd.read_excel(pred_openai_nano_zs_path)

# ========= 2. Merge on File Name (OpenAI GPT-4 Nano Zero-Shot) =========
df_merged = pd.merge(
    df_gt,
    df_pred_openai_nano_zs,
    left_on="File Name",
...
print(f"Total matched records (OpenAI GPT-4 Nano Zero-Shot): {len(df_merged)}\n")
...
print("Normalizing OpenAI GPT-4 Nano Zero-Shot data...")
...
for idx, (target_name, (gt_col, pred_col)) in enumerate(targets.items()):
    ...
    ax.set_xlabel("Predicted (OpenAI GPT-4 Nano Zero-Shot)", fontsize=11, fontweight='bold')
    ...
    plt.xlabel("Predicted (OpenAI GPT-4 Nano Zero-Shot)", fontsize=12, fontweight='bold')
    plt.ylabel("Ground Truth", fontsize=12, fontweight='bold')
    plt.title(f"Confusion Matrix: {target_name} (OpenAI GPT-4 Nano Zero-Shot)\n(Accuracy = {acc:.2%})", 
              fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(f"confusion_matrix_{target_name.replace(' ', '_')}_openai_nano_zs.png", 
                dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"\n✓ Saved: confusion_matrix_{target_name.replace(' ', '_')}_openai_nano_zs.png")

# Save combined figure
plt.figure(fig.number)
plt.tight_layout()
plt.savefig("confusion_matrices_all_openai_nano_zs.png", dpi=300, bbox_inches='tight')
plt.close()

# ========= 6. Summary Report =========
print(f"\n{'='*70}")
print("SUMMARY - ACCURACY SCORES (OPENAI GPT-4 NANO ZERO-SHOT)")
print(f"{'='*70}")
...
summary_df.to_csv("accuracy_summary_openai_nano_zs.csv", index=False)
print(f"\n✓ Summary saved to: accuracy_summary_openai_nano_zs.csv")
...
print("\n✓ Analysis complete!")
print(f"\nGenerated Files:")
print("  1. confusion_matrix_HER2_openai_nano_zs.png")
print("  2. confusion_matrix_ER_openai_nano_zs.png")
print("  3. confusion_matrix_PR_openai_nano_zs.png")
print("  4. confusion_matrix_Tumor_Size_openai_nano_zs.png")
print("  5. confusion_matrices_all_openai_nano_zs.png (combined)")
print("  6. accuracy_summary_openai_nano_zs.csv")