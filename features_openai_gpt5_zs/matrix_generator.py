...
pred_gpt5_path = "features_openai_gpt5_zs.xlsx"

df_gt = pd.read_excel(gt_path)
df_pred_gpt5 = pd.read_excel(pred_gpt5_path)

# ========= 2. Merge on File Name (OpenAI GPT-5) =========
df_merged = pd.merge(
    df_gt,
    df_pred_gpt5,
    left_on="File Name",
...
print(f"Total matched records (OpenAI GPT-5): {len(df_merged)}\n")
...
print("Normalizing OpenAI GPT-5 data...")
...
for idx, (target_name, (gt_col, pred_col)) in enumerate(targets.items()):
    ...
    ax.set_xlabel("Predicted (OpenAI GPT-5)", fontsize=11, fontweight='bold')
    ...
    plt.xlabel("Predicted (OpenAI GPT-5)", fontsize=12, fontweight='bold')
    plt.ylabel("Ground Truth", fontsize=12, fontweight='bold')
    plt.title(f"Confusion Matrix: {target_name} (OpenAI GPT-5)\n(Accuracy = {acc:.2%})", 
              fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(f"confusion_matrix_{target_name.replace(' ', '_')}_gpt5.png", 
                dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"\n✓ Saved: confusion_matrix_{target_name.replace(' ', '_')}_gpt5.png")

# Save combined figure
plt.figure(fig.number)
plt.tight_layout()
plt.savefig("confusion_matrices_all_gpt5.png", dpi=300, bbox_inches='tight')
plt.close()

# ========= 6. Summary Report =========
print(f"\n{'='*70}")
print("SUMMARY - ACCURACY SCORES (OPENAI GPT-5)")
print(f"{'='*70}")
...
summary_df.to_csv("accuracy_summary_gpt5.csv", index=False)
print(f"\n✓ Summary saved to: accuracy_summary_gpt5.csv")
...
print("\n✓ Analysis complete!")
print(f"\nGenerated Files:")
print("  1. confusion_matrix_HER2_gpt5.png")
print("  2. confusion_matrix_ER_gpt5.png")
print("  3. confusion_matrix_PR_gpt5.png")
print("  4. confusion_matrix_Tumor_Size_gpt5.png")
print("  5. confusion_matrices_all_gpt5.png (combined)")
print("  6. accuracy_summary_gpt5.csv")