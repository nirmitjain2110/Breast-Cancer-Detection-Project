...
pred_openai_path = "features_openai _4.1_ct.xlsx"

df_gt = pd.read_excel(gt_path)
df_pred_openai = pd.read_excel(pred_openai_path)

# ========= 2. Merge on File Name (OpenAI GPT-4) =========
df_merged = pd.merge(
    df_gt,
    df_pred_openai,
    left_on="File Name",
...
print(f"Total matched records (OpenAI GPT-4): {len(df_merged)}\n")
...
print("Normalizing OpenAI GPT-4 data...")
...
for idx, (target_name, (gt_col, pred_col)) in enumerate(targets.items()):
    ...
    ax.set_xlabel("Predicted (OpenAI GPT-4)", fontsize=11, fontweight='bold')
    ...
    plt.xlabel("Predicted (OpenAI GPT-4)", fontsize=12, fontweight='bold')
    plt.ylabel("Ground Truth", fontsize=12, fontweight='bold')
    plt.title(f"Confusion Matrix: {target_name} (OpenAI GPT-4)\n(Accuracy = {acc:.2%})", 
              fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(f"confusion_matrix_{target_name.replace(' ', '_')}_openai.png", 
                dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"\n✓ Saved: confusion_matrix_{target_name.replace(' ', '_')}_openai.png")

# Save combined figure
plt.figure(fig.number)
plt.tight_layout()
plt.savefig("confusion_matrices_all_openai.png", dpi=300, bbox_inches='tight')
plt.close()

# ========= 6. Summary Report =========
print(f"\n{'='*70}")
print("SUMMARY - ACCURACY SCORES (OPENAI GPT-4)")
print(f"{'='*70}")
...
summary_df.to_csv("accuracy_summary_openai.csv", index=False)
print(f"\n✓ Summary saved to: accuracy_summary_openai.csv")
...
print("\n✓ Analysis complete!")
print(f"\nGenerated Files:")
print("  1. confusion_matrix_HER2_openai.png")
print("  2. confusion_matrix_ER_openai.png")
print("  3. confusion_matrix_PR_openai.png")
print("  4. confusion_matrix_Tumor_Size_openai.png")
print("  5. confusion_matrices_all_openai.png (combined)")
print("  6. accuracy_summary_openai.csv")