...
for idx, (target_name, (gt_col, pred_col)) in enumerate(targets.items()):
    ...
    ax.set_xlabel("Predicted (Gemini)", fontsize=11, fontweight='bold')
    ...
    plt.xlabel("Predicted (Gemini)", fontsize=12, fontweight='bold')
    plt.ylabel("Ground Truth", fontsize=12, fontweight='bold')
    plt.title(f"Confusion Matrix: {target_name} (Gemini)\n(Accuracy = {acc:.2%})", 
              fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(f"confusion_matrix_{target_name.replace(' ', '_')}_gemini.png", 
                dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"\n✓ Saved: confusion_matrix_{target_name.replace(' ', '_')}_gemini.png")

# Save combined figure
plt.figure(fig.number)
plt.tight_layout()
plt.savefig("confusion_matrices_all_gemini.png", dpi=300, bbox_inches='tight')
plt.close()

# ========= 6. Summary Report =========
print(f"\n{'='*70}")
print("SUMMARY - ACCURACY SCORES (GEMINI)")
print(f"{'='*70}")
...
summary_df.to_csv("accuracy_summary_gemini.csv", index=False)
print(f"\n✓ Summary saved to: accuracy_summary_gemini.csv")
...
print("\n✓ Analysis complete!")
print(f"\nGenerated Files:")
print("  1. confusion_matrix_HER2_gemini.png")
print("  2. confusion_matrix_ER_gemini.png")
print("  3. confusion_matrix_PR_gemini.png")
print("  4. confusion_matrix_Tumor_Size_gemini.png")
print("  5. confusion_matrices_all_gemini.png (combined)")
print("  6. accuracy_summary_gemini.csv")