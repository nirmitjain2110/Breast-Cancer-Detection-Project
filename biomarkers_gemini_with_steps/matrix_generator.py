...
pred_biomarkers_gemini_steps_path = "biomarkers_gemini_with_steps.xlsx"

df_gt = pd.read_excel(gt_path)
df_pred_biomarkers_gemini_steps = pd.read_excel(pred_biomarkers_gemini_steps_path)

# ========= 2. Merge on File Name (Biomarkers Gemini with Steps) =========
df_merged = pd.merge(
    df_gt,
    df_pred_biomarkers_gemini_steps,
    left_on="File Name",
...
print(f"Total matched records (Biomarkers Gemini with Steps): {len(df_merged)}\n")
...
print("Normalizing Biomarkers Gemini with Steps data...")
...
for idx, (target_name, (gt_col, pred_col)) in enumerate(targets.items()):
    ...
    ax.set_xlabel("Predicted (Biomarkers Gemini w/ Steps)", fontsize=11, fontweight='bold')
    ...
    plt.xlabel("Predicted (Biomarkers Gemini w/ Steps)", fontsize=12, fontweight='bold')
    plt.ylabel("Ground Truth", fontsize=12, fontweight='bold')
    plt.title(f"Confusion Matrix: {target_name} (Biomarkers Gemini with Steps)\n(Accuracy = {acc:.2%})", 
              fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(f"confusion_matrix_{target_name.replace(' ', '_')}_biomarkers_gemini_steps.png", 
                dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"\n✓ Saved: confusion_matrix_{target_name.replace(' ', '_')}_biomarkers_gemini_steps.png")

# Save combined figure
plt.figure(fig.number)
plt.tight_layout()
plt.savefig("confusion_matrices_all_biomarkers_gemini_steps.png", dpi=300, bbox_inches='tight')
plt.close()

# ========= 6. Summary Report =========
print(f"\n{'='*70}")
print("SUMMARY - ACCURACY SCORES (BIOMARKERS GEMINI WITH STEPS)")
print(f"{'='*70}")
...
summary_df.to_csv("accuracy_summary_biomarkers_gemini_steps.csv", index=False)
print(f"\n✓ Summary saved to: accuracy_summary_biomarkers_gemini_steps.csv")
...
print("\n✓ Analysis complete!")
print(f"\nGenerated Files:")
print("  1. confusion_matrix_HER2_biomarkers_gemini_steps.png")
print("  2. confusion_matrix_ER_biomarkers_gemini_steps.png")
print("  3. confusion_matrix_PR_biomarkers_gemini_steps.png")
print("  4. confusion_matrix_Tumor_Size_biomarkers_gemini_steps.png")
print("  5. confusion_matrices_all_biomarkers_gemini_steps.png (combined)")
print("  6. accuracy_summary_biomarkers_gemini_steps.csv")