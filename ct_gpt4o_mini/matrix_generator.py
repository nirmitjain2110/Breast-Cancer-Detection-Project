...
pred_ct_path = "ct_gpt4o_mini.xlsx"

df_gt = pd.read_excel(gt_path)
df_pred_ct = pd.read_excel(pred_ct_path)

# ========= 2. Merge on File Name (CT) =========
df_merged = pd.merge(
    df_gt,
    df_pred_ct,
    left_on="File Name",
...
print(f"Total matched records (CT): {len(df_merged)}\n")
...
for idx, (target_name, (gt_col, pred_col)) in enumerate(targets.items()):
    ...
    ax.set_xlabel("Predicted (GPT-4o Mini - CT)", fontsize=11, fontweight='bold')
    ...
    plt.xlabel("Predicted (GPT-4o Mini - CT)", fontsize=12, fontweight='bold')
    plt.ylabel("Ground Truth", fontsize=12, fontweight='bold')
    plt.title(f"Confusion Matrix: {target_name} (CT)\n(Accuracy = {acc:.2%})", 
              fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(f"confusion_matrix_{target_name.replace(' ', '_')}_ct.png", 
                dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"\n✓ Saved: confusion_matrix_{target_name.replace(' ', '_')}_ct.png")

# Save combined figure
plt.figure(fig.number)
plt.tight_layout()
plt.savefig("confusion_matrices_all_ct.png", dpi=300, bbox_inches='tight')
plt.close()

# ========= 6. Summary Report =========
print(f"\n{'='*70}")
print("SUMMARY - ACCURACY SCORES (CT)")
print(f"{'='*70}")
...
summary_df.to_csv("accuracy_summary_ct.csv", index=False)
print(f"\n✓ Summary saved to: accuracy_summary_ct.csv")
...
print("\n✓ Analysis complete!")
print(f"\nGenerated Files:")
print("  1. confusion_matrix_HER2_ct.png")
print("  2. confusion_matrix_ER_ct.png")
print("  3. confusion_matrix_PR_ct.png")
print("  4. confusion_matrix_Tumor_Size_ct.png")
print("  5. confusion_matrices_all_ct.png (combined)")
print("  6. accuracy_summary_ct.csv")