...
pred_os_path = "features_openai_os.xlsx"

df_gt = pd.read_excel(gt_path)
df_pred_os = pd.read_excel(pred_os_path)

# ========= 2. Merge on File Name (OpenAI O-Series) =========
df_merged = pd.merge(
    df_gt,
    df_pred_os,
    left_on="File Name",
...
print(f"Total matched records (OpenAI O-Series): {len(df_merged)}\n")
...
print("Normalizing OpenAI O-Series data...")
...
for idx, (target_name, (gt_col, pred_col)) in enumerate(targets.items()):
    ...
    ax.set_xlabel("Predicted (OpenAI O-Series)", fontsize=11, fontweight='bold')
    ...
    plt.xlabel("Predicted (OpenAI O-Series)", fontsize=12, fontweight='bold')
    plt.ylabel("Ground Truth", fontsize=12, fontweight='bold')
    plt.title(f"Confusion Matrix: {target_name} (OpenAI O-Series)\n(Accuracy = {acc:.2%})", 
              fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(f"confusion_matrix_{target_name.replace(' ', '_')}_os.png", 
                dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"\n✓ Saved: confusion_matrix_{target_name.replace(' ', '_')}_os.png")

# Save combined figure
plt.figure(fig.number)
plt.tight_layout()
plt.savefig("confusion_matrices_all_os.png", dpi=300, bbox_inches='tight')
plt.close()

# ========= 6. Summary Report =========
print(f"\n{'='*70}")
print("SUMMARY - ACCURACY SCORES (OPENAI O-SERIES)")
print(f"{'='*70}")
...
summary_df.to_csv("accuracy_summary_os.csv", index=False)
print(f"\n✓ Summary saved to: accuracy_summary_os.csv")
...
print("\n✓ Analysis complete!")
print(f"\nGenerated Files:")
print("  1. confusion_matrix_HER2_os.png")
print("  2. confusion_matrix_ER_os.png")
print("  3. confusion_matrix_PR_os.png")
print("  4. confusion_matrix_Tumor_Size_os.png")
print("  5. confusion_matrices_all_os.png (combined)")
print("  6. accuracy_summary_os.csv")