...
pred_biomarkers_gemini_path = "biomarkers_gemini.xlsx"

df_gt = pd.read_excel(gt_path)
df_pred_biomarkers_gemini = pd.read_excel(pred_biomarkers_gemini_path)

# ========= 2. Merge on File Name (Biomarkers Gemini) =========
df_merged = pd.merge(
    df_gt,
    df_pred_biomarkers_gemini,
    left_on="File Name",
...
print(f"Total matched records (Biomarkers Gemini): {len(df_merged)}\n")
...
print("Normalizing Biomarkers Gemini data...")
...
for idx, (target_name, (gt_col, pred_col)) in enumerate(targets.items()):
    ...
    ax.set_xlabel("Predicted (Biomarkers Gemini)", fontsize=11, fontweight='bold')
    ...
    plt.xlabel("Predicted (Biomarkers Gemini)", fontsize=12, fontweight='bold')
    plt.ylabel("Ground Truth", fontsize=12, fontweight='bold')
    plt.title(f"Confusion Matrix: {target_name} (Biomarkers Gemini)\n(Accuracy = {acc:.2%})", 
              fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(f"confusion_matrix_{target_name.replace(' ', '_')}_biomarkers_gemini.png", 
                dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"\n✓ Saved: confusion_matrix_{target_name.replace(' ', '_')}_biomarkers_gemini.png")

# Save combined figure
plt.figure(fig.number)
plt.tight_layout()
plt.savefig("confusion_matrices_all_biomarkers_gemini.png", dpi=300, bbox_inches='tight')
plt.close()

# ========= 6. Summary Report =========
print(f"\n{'='*70}")
print("SUMMARY - ACCURACY SCORES (BIOMARKERS GEMINI)")
print(f"{'='*70}")
...
summary_df.to_csv("accuracy_summary_biomarkers_gemini.csv", index=False)
print(f"\n✓ Summary saved to: accuracy_summary_biomarkers_gemini.csv")
...
print("\n✓ Analysis complete!")
print(f"\nGenerated Files:")
print("  1. confusion_matrix_HER2_biomarkers_gemini.png")
print("  2. confusion_matrix_ER_biomarkers_gemini.png")
print("  3. confusion_matrix_PR_biomarkers_gemini.png")
print("  4. confusion_matrix_Tumor_Size_biomarkers_gemini.png")
print("  5. confusion_matrices_all_biomarkers_gemini.png (combined)")
print("  6. accuracy_summary_biomarkers_gemini.csv")