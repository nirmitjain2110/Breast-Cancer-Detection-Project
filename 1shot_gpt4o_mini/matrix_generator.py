...
pred_1shot_path = "1shot_gpt4o_mini.xlsx"

df_gt = pd.read_excel(gt_path)
df_pred_1shot = pd.read_excel(pred_1shot_path)

# ========= 2. Merge on File Name (1-shot) =========
df_merged = pd.merge(
    df_gt,
    df_pred_1shot,
    left_on="File Name",
...
print(f"Total matched records (1-shot): {len(df_merged)}\n")
...
for idx, (target_name, (gt_col, pred_col)) in enumerate(targets.items()):
    ...
    ax.set_xlabel("Predicted (GPT-4o Mini - 1-shot)", fontsize=11, fontweight='bold')
    ...
    plt.xlabel("Predicted (GPT-4o Mini - 1-shot)", fontsize=12, fontweight='bold')
    plt.ylabel("Ground Truth", fontsize=12, fontweight='bold')
    plt.title(f"Confusion Matrix: {target_name} (1-shot)\n(Accuracy = {acc:.2%})", 
              fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(f"confusion_matrix_{target_name.replace(' ', '_')}_1shot.png", 
                dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"\n✓ Saved: confusion_matrix_{target_name.replace(' ', '_')}_1shot.png")

# Save combined figure
plt.figure(fig.number)
plt.tight_layout()
plt.savefig("confusion_matrices_all_1shot.png", dpi=300, bbox_inches='tight')
plt.close()

# ========= 6. Summary Report =========
print(f"\n{'='*70}")
print("SUMMARY - ACCURACY SCORES (1-SHOT)")
print(f"{'='*70}")
...
summary_df.to_csv("accuracy_summary_1shot.csv", index=False)
print(f"\n✓ Summary saved to: accuracy_summary_1shot.csv")
...
print("\n✓ Analysis complete!")
print(f"\nGenerated Files:")
print("  1. confusion_matrix_HER2_1shot.png")
print("  2. confusion_matrix_ER_1shot.png")
print("  3. confusion_matrix_PR_1shot.png")
print("  4. confusion_matrix_Tumor_Size_1shot.png")
print("  5. confusion_matrices_all_1shot.png (combined)")
print("  6. accuracy_summary_1shot.csv")